...
description: 先执行 /byd:dev-plan 规划，再按已确认方案实现ssg页面或ssr页面的nextjs代码
---

# BYDFi Next.js 代码实现

请按 `.claude/skills/dev/nextjs-dev/SKILL.md` 的“实现”流程执行，并将 `$ARGUMENTS` 按 `<文件路径> | <需求说明>` 解析为两个输入：
...
- api
- utilsc
- hooks（涉及 hooks 时必须按 `.claude/knowledge/discovery-hooks.md` 执行，优先复用 `packages/apps-kit/core/hooks`）
- store
- shared
- formulas
- events
...
请按 `.claude/knowledge/discovery-api.md` 的“查找可复用的接口”流程执行。
...
- 新增 hook 前必须说明已查 `.claude/knowledge/discovery-hooks.md` 与 `packages/apps-kit/core/hooks`；已有能力优先复用。
- 不擅自新增依赖。
- 完成后必须给出验证结果或未验证原因。
