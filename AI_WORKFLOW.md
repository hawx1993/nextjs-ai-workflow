...
```text
/byd:dev-plan 说明需求、目标页面、期望交互、涉及 app
```
...
```text
/byd:dev-implement apps/byd-ssg/src/pages/xxx/index.page.tsx 按已确认方案实现
```

完成后执行：

```text
/byd:dev-review apps/byd-ssg/src/pages/xxx/index.page.tsx
/byd:dev-verify apps/byd-ssg/src/pages/xxx/index.page.tsx
```

### 2.2 Bug 修复

```text
/byd:dev-plan 修复 xxx 页面在 RTL 下按钮错位的问题，范围限定 apps/byd-ssg
/byd:dev-implement <确认后的文件路径和修复说明>
/byd:dev-review <文件路径>
/byd:dev-verify <范围>
```
...
```text
/byd:dev-fix-build 粘贴 tsc 或 build 错误输出
```
...
```text
/byd:dev-review 当前 diff
/byd:dev-review apps/byd-ssr/src/pages/xxx/index.page.tsx
```
...
| 命令                 | 用途                    |
| -------------------- | ----------------------- |
| `/byd:dev-plan`      | 规划，不改代码          |
| `/byd:dev-implement` | 实现，限 Next/React/TSX |
| `/byd:dev-review`    | 审查 diff 或文件        |
| `/byd:dev-fix-build` | 修构建/类型错误         |
| `/byd:dev-verify`    | 验证与汇报              |

专项命令继续保留：

- 审计类：`/byd:audit-*`
- 代码生成类：`/byd:codegen-*`
- Git/PR 类：`/byd:workflow-git-*`
- 发布前类：`/byd:workflow-pre-release`
...
Agent 是“角色说明书”，不是替代规则。执行时仍必须遵守 `.claude/rules/*`。
...
- 页面/路由/数据流：`.claude/rules/nextjs.md`
- 组件/hooks：`.claude/rules/react-tsx.md`
- 类型/计算/import：`.claude/rules/typescript.md`
- 跨包共享：`.claude/rules/monorepo.md`
- 文案：`.claude/rules/language.md`
- 样式：`.claude/rules/theme.md`
- 验证：`.claude/rules/verification.md`
