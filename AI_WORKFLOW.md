...
```text
/byd-next-workflow:byd:dev-plan 说明需求、目标页面、期望交互、涉及 app
```
...
```text
/byd-next-workflow:byd:dev-implement apps/byd-ssg/src/pages/xxx/index.page.tsx 按已确认方案实现
```

完成后执行：

```text
/byd-next-workflow:byd:dev-review apps/byd-ssg/src/pages/xxx/index.page.tsx
/byd-next-workflow:byd:dev-verify apps/byd-ssg/src/pages/xxx/index.page.tsx
```

### 2.2 Bug 修复

```text
/byd-next-workflow:byd:dev-plan 修复 xxx 页面在 RTL 下按钮错位的问题，范围限定 apps/byd-ssg
/byd-next-workflow:byd:dev-implement <确认后的文件路径和修复说明>
/byd-next-workflow:byd:dev-review <文件路径>
/byd-next-workflow:byd:dev-verify <范围>
```
...
```text
/byd-next-workflow:byd:dev-fix-build 粘贴 tsc 或 build 错误输出
```
...
```text
/byd-next-workflow:byd:dev-review 当前 diff
/byd-next-workflow:byd:dev-review apps/byd-ssr/src/pages/xxx/index.page.tsx
```
...
```text
/byd-next-workflow:byd:dev-verify 当前改动
```
...
| 命令                 | 用途                    |
| -------------------- | ----------------------- |
| `/byd-next-workflow:byd:dev-plan`      | 规划，不改代码          |
| `/byd-next-workflow:byd:dev-implement` | 实现，限 Next/React/TSX |
| `/byd-next-workflow:byd:dev-review`    | 审查 diff 或文件        |
| `/byd-next-workflow:byd:dev-fix-build` | 修构建/类型错误         |
| `/byd-next-workflow:byd:dev-verify`    | 验证与汇报              |

专项命令继续保留：

- 审计类：`/byd-next-workflow:byd:audit-*`
- 代码生成类：`/byd-next-workflow:byd:codegen-*`
- Git/PR 类：`/byd-next-workflow:byd:workflow-git-*`
- 发布前类：`/byd-next-workflow:byd:workflow-pre-release`
...
```text
/byd-next-workflow:byd:dev-plan
目标：给合约交易页新增风险提示卡片
范围：apps/byd-ssg
